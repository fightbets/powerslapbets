...
export default function ShareBar({ post }: { post: Post }) {
  const url = encodeURIComponent(`https://powerslapbets.com/post/${post.slug}`)
  const text = encodeURIComponent(`${post.emoji} ${post.title} — PowerSlapBets`)
  ...
  const shares = [
    ...
    },
    {
      ...
      icon: '📋',
      onClick: () => {
        navigator.clipboard.writeText(`https://powerslapbets.com/post/${post.slug}`)
        alert('Link copied!')
      },
    },
  ]
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs text-muted font-medium uppercase tracking-wider">Share:</span>
      {shares.map(s => (
        <a
          key={s.name}
          href={s.href}
